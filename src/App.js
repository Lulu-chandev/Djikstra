...
// Data lokasi kampus (nodes) - SUDAH DIPERBAIKI
const locations = {
  'Gerbang': { x: 100, y: 100, id: 'gb' },
  'Pos Satpam': { x: 200, y: 150, id: 'ps' },
  'Kantin': { x: 300, y: 150, id: 'kt' },
  ...
  'Gedung Utama': { x: 600, y: 300, id: 'gu' },
  ...
  useEffect(() => {
    ...
    if (result && result.path.length > 1) {
      ...
      for (let i = 0; i < result.path.length - 1; i++) {
        ...
        // Tampilkan label jarak di tengah garis
        ...
        ctx.fillRect(midX - 18 * scale, midY - 10 * scale, 36 * scale, 20 * scale);
        
        // Border biru untuk label
        ctx.strokeStyle = '#3b82f6';
        ctx.lineWidth = 2 * scale;
        ctx.strokeRect(midX - 18 * scale, midY - 10 * scale, 36 * scale, 20 * scale);
